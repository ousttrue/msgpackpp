...
class transport : std::enable_shared_from_this<transport> {
  ...
  transport(const on_message_func &callback) : _callback(callback) {}
...
public:
  ...
  void start_read() {
    ...
    auto on_read = [shared =
      ...
      if (error && (error != ::asio::error::eof)) {
        ...
        shared->_buffer.erase(shared->_buffer.begin(),
                              shared->_buffer.begin() + d);

        // next loop
        shared->start_read();
      }
    };
    _socket->async_read_some(asio::buffer(_buffer), on_read);
  }
  ...
  void write_async(const bytes &bytes) {
    ...
    _socket->async_write_some(::asio::buffer(bytes.data(), bytes.size()),
                              on_write);
  }

  static std::shared_ptr<transport>
  connect_async(asio::io_service &io_service,
  ...
  std::future<R> call(const std::string &method, ARGS...) {
    ...
    _call_map.insert(std::make_pair(
        1, [p](const msgpackpp::parser &msg) { p->set_value({}); }));
...
public:
  ...
  void listen(const asio::ip::tcp::endpoint &ep,
              const on_message_func &callback) {
...
int main(int argc, char **argv) {
  ...
  std::function<void(int)> setter =
      std::bind(&SomeClass::setNumber, &s, std::placeholders::_1);
  ...
  server.listen(
      ...
      [&dispatcher](const msgpackpp::parser &msg) -> msgpackpp::bytes {
  ...
  auto client = msgpackpp::rpc::transport::connect_async(
      client_io, asio::ip::tcp::endpoint(
